/* js/cart.js - logika koszyka dla cart.html z synchronizacją */
(function(){
  'use strict';
  ...
  function formatPrice(n) { return n.toLocaleString('pl-PL', { style: 'currency', currency: 'PLN' }); }
  ...
  function attachEvents() {
    ...
    if (clearBtn) clearBtn.onclick = () => { if (confirm('Czy na pewno wyczyścić koszyk?')) { localStorage.removeItem(CART_KEY); renderCart(); try { if (bc) bc.postMessage({ type: 'cart_updated' }); } catch(e){} } };
  ...
  function renderCart() {
    ...
    if (cart.length === 0) { emptyInfo.style.display = 'block'; container.appendChild(emptyInfo); }
    else {
  ...
  function onStorageEvent(e) { if (!e) return; if (e.key === CART_KEY) try{ renderCart(); }catch(e){} }
  function onBroadcastMessage(ev) { try{ if (!ev||!ev.data) return; if (ev.data.type==='cart_updated') renderCart(); }catch(e){} }

  document.addEventListener('DOMContentLoaded', function(){ renderCart(); window.addEventListener('storage', onStorageEvent); if (bc) bc.addEventListener('message', onBroadcastMessage); });
